    ...
    <div class="fade-in">
        <div class="mb-6 flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-gray-500">Communicate with parents and administration</p>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead class="bg-gray-50 border-b border-gray-100">
                        <tr>
                            <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase">From</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($messages as $message)
                            <tr class="hover:bg-gray-50 transition {{ !$message->is_read ? 'bg-indigo-50/30' : '' }}">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div
                                                class="text-sm font-medium text-gray-800 {{ !$message->is_read ? 'font-bold' : '' }}">
                                                {{ $message->sender->name }}
                                            </div>
                                            <div class="text-xs text-gray-400 capitalize">{{ $message->sender->role }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-800 {{ !$message->is_read ? 'font-bold' : '' }}">
                                        {{ $message->subject }}
                                    </div>
                                    <div class="text-xs text-gray-400 truncate max-w-xs">{{ $message->message }}</div>
                                </td>
