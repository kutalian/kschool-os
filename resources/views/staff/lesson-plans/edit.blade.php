<x-master-layout>
    <div class="fade-in">
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500">Update your teaching strategy for "{{ $lessonPlan->topic }}"</p>
        </div>

        <div class="max-w-4xl">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
                <form action="{{ route('staff.lesson-plans.update', $lessonPlan) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="week_start_date" class="block mt-1 w-full" type="date"
                                name="week_start_date" :value="old('week_start_date', $lessonPlan->week_start_date->format('Y-m-d'))" required />
                        </div>
                    </div>

                    <div>
                        <x-input-label for="topic" :value="__('Topic Title')" />
                        <x-text-input id="topic" class="block mt-1 w-full" type="text" name="topic" :value="old('topic', $lessonPlan->topic)" required />
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea id="objectives" name="objectives" rows="4"
                                class="mt-1 block w-full border-gray-300 focus:border-green-500 focus:ring-green-500 rounded-lg shadow-sm">{{ old('objectives', $lessonPlan->objectives) }}</textarea>
                        </div>
                        <div>
                            <x-input-label for="activities" :value="__('Classroom Activities')" />
                            <textarea id="activities" name="activities" rows="4"
                                class="mt-1 block w-full border-gray-300 focus:border-green-500 focus:ring-green-500 rounded-lg shadow-sm">{{ old('activities', $lessonPlan->activities) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="homework" name="homework" rows="2"
                            class="mt-1 block w-full border-gray-300 focus:border-green-500 focus:ring-green-500 rounded-lg shadow-sm">{{ old('homework', $lessonPlan->homework) }}</textarea>
                    </div>
