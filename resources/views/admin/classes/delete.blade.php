<x-master-layout>
    <div class="flex items-center justify-center min-h-[80vh]">
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-8 max-w-lg w-full text-center">
            ...
            <h2 class="text-2xl font-bold text-gray-800 mb-2">Delete Class?</h2>

            @if($studentsCount > 0)
                <p class="text-red-600 font-bold mb-6">Action Blocked: Students Enrolled.</p>
            @else
                <p class="text-gray-500 mb-6">Are you sure you want to delete this class?</p>
            @endif

            <div class="bg-gray-50 rounded-lg p-4 mb-6 text-left border border-gray-100">
                <div class="flex items-center">
                    <div
                        class="h-10 w-10 rounded-full bg-green-100 flex items-center justify-center text-green-600 font-bold mr-3">
                        <i class="fas fa-chalkboard"></i>
                    </div>
                    <div>
                        <h3 class="font-bold text-gray-900">{{ $classRoom->name }} {{ $classRoom->section }}</h3>
                        <p class="text-sm text-gray-500">Teacher:
                            {{ $classRoom->teacher ? $classRoom->teacher->name : 'Not Assigned' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="text-left mb-6">
                ...
                <div
                    class="bg-yellow-50 rounded-lg p-3 border border-yellow-100 mb-2 flex justify-between items-center text-yellow-800">
                    <div class="flex items-center">
                ...
                <div
                    ...
                    <span
                        class="bg-yellow-200 text-yellow-900 py-1 px-3 rounded-full text-xs font-bold">{{ $subjectsCount }}</span>
                </div>
            </div>
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-2 text-sm text-red-700">
                            @if($studentsCount > 0)
                                <p>You <strong>CANNOT</strong> delete this class because students are currently enrolled.
                                </p>
                            @else
                                <p>This will permanently delete the class and unassign all subjects from it. This action
                                    cannot be undone.</p>
                            @endif
                        </div>
                    </div>
            ...
            <div class="flex space-x-4">
                ...
                @if($studentsCount == 0)
                    <form action="{{ route('classes.destroy', $classRoom->id) }}" method="POST" class="w-1/2">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
