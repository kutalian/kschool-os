<x-master-layout>
    <div class="flex items-center justify-center min-h-[80vh]">
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-8 max-w-lg w-full text-center">
            ...
            <p class="text-gray-500 mb-6">Are you sure you want to delete this health record?</p>
            ...
            <div class="bg-gray-50 rounded-lg p-4 mb-6 text-left border border-gray-100">
                <div class="flex items-center">
                    ...
                    <div>
                        <h3 class="font-bold text-gray-900">{{ $healthRecord->record_type }}</h3>
                        <p class="text-sm text-gray-500">Student: {{ $healthRecord->student->name }}</p>
                        <p class="text-xs text-gray-400">Date: {{ $healthRecord->record_date->format('M d, Y') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex space-x-4">
                ...
                <form action="{{ route('health-records.destroy', $healthRecord->id) }}" method="POST" class="w-1/2">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
