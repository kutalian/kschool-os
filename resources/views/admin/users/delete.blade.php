<x-master-layout>
    <div class="flex items-center justify-center min-h-[80vh]">
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-8 max-w-lg w-full text-center">
            ...
            <h2 class="text-2xl font-bold text-gray-800 mb-2">Delete User Account?</h2>
            ...
            <div class="bg-gray-50 rounded-lg p-4 mb-6 text-left border border-gray-100">
                <div class="flex items-center">
                    <div
                        class="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold mr-3">
                        {{ substr($user->username, 0, 1) }}
                    </div>
                    <div>
                        <h3 class="font-bold text-gray-900">{{ $user->username }}</h3>
                        <p class="text-sm text-gray-500">Email: {{ $user->email }} | Role: {{ ucfirst($user->role) }}
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-2 text-sm text-red-700">
                            <p>This action will permanently delete the user account. If this user is linked to a staff,
                                student, or parent profile, that profile will become an orphan or may cause errors.
            ...
            <div class="flex space-x-4">
                ...
                <form action="{{ route('users.destroy', $user->id) }}" method="POST" class="w-1/2">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
