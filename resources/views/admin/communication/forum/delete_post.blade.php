<x-master-layout>
    <div class="flex items-center justify-center min-h-[80vh]">
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-8 max-w-lg w-full text-center">
            ...
            <h2 class="text-2xl font-bold text-gray-800 mb-2">Delete Forum Post?</h2>
            <p class="text-gray-500 mb-6">Are you sure you want to delete this discussion? This action will also delete
                all comments associated with it.</p>

            <div class="bg-gray-50 rounded-lg p-4 mb-6 text-left border border-gray-100">
                <div class="flex items-start">
                    <div
                        class="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold mr-3 mt-1 shrink-0">
                        <i class="fas fa-comments"></i>
                    </div>
                    <div>
                        <h3 class="font-bold text-gray-900 line-clamp-2">{{ $forum->title }}</h3>
                        <p class="text-sm text-gray-500">
                            By {{ $forum->is_anonymous ? 'Anonymous' : $forum->user->name }} |
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-2 text-sm text-red-700">
                            <p>This will permanently delete the post, its <strong>{{ $forum->comments()->count() }}
                                    comments</strong>, and all associated data.</p>
                            @if($forum->poll)
                                <p class="mt-1">The associated poll <strong>"{{ $forum->poll->question }}"</strong> and its
                                    <strong>{{ $forum->poll->votes()->count() }} votes</strong> will also be removed.</p>
                            @endif
            ...
            <div class="flex space-x-4">
                ...
                <form action="{{ route('forum.destroy', $forum->id) }}" method="POST" class="w-1/2">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
