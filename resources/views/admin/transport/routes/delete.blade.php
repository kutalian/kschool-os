            ...
            <div class="md:flex">
                <div class="w-full p-4">
                    <div class="text-center mb-6">
                        ...
                        <h1 class="block text-gray-700 font-bold text-xl mb-2">Delete Route?</h1>
                        <p class="text-gray-600">Are you sure you want to delete route
                            <strong>{{ $transportRoute->route_name }}</strong>?
                        </p>
                        <p class="text-sm text-gray-500 mt-2">{{ $transportRoute->start_point }} to
                            {{ $transportRoute->end_point }}
                        </p>
                    </div>

                    <form action="{{ route('transport.routes.destroy', $transportRoute->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
